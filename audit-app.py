...
def main():
    ...
    def _busy_gate():
        """Full-page notice shown to a session that does NOT own the live run.

        The registry allows one audit process-wide; a different session (shared
        password) is running it. Poll so the banner tracks progress and, when
        the other run ends, rerun the whole app to restore this session's UI.
        """
        active = registry.active()
        if active is None or active.run_id == st.session_state.get("active_run_id"):
            st.rerun(scope="app")
            return
        snap = active.snapshot()
        cur, tot = snap.progress_current, snap.progress_total
        elapsed_min = max(0, int((time.time() - snap.started_at) // 60))
        phase = "summarization " if snap.status == audit_worker.JobStatus.RUNNING_SUMMARY else ""
        detail = f" — {phase}{int(100 * cur / tot)}% complete ({cur} of {tot} topics)" if tot else ""
        running_for = f" It's been running for about {elapsed_min} min." if elapsed_min else ""
        st.info(
            f"**An audit is already running** in another session{detail}."
            f"{running_for} This page will reload automatically when the other audit finishes."
        )
        if tot:
            st.progress(cur / tot)

    @st.fragment(run_every="2s")
    def _collision_heartbeat():
        """Idle session: flip to the busy gate within ~2s of another session
        grabbing the audit slot, instead of waiting for a manual interaction."""
        active = registry.active()
        if active is not None and active.run_id != st.session_state.get("active_run_id"):
            st.rerun(scope="app")

    # --- Single-active-run collision gate (before the rest of the UI) ------
    # Re-attach this tab to a run it started (the URL token survives a
    # reconnect). If a DIFFERENT session holds the one audit slot, show a
    # full-page busy notice and render nothing else.
    _active_job = registry.active()
    if _active_job is not None and not st.session_state.get("active_run_id"):
        if st.query_params.get("run") == _active_job.run_id:
            st.session_state["active_run_id"] = _active_job.run_id
    if (
        _active_job is not None
        and _active_job.run_id != st.session_state.get("active_run_id")
    ):
        st.session_state["audit_run_requested"] = False
        _busy_gate()
        return
    ...
    def _launch_audit_job():
        ...
        def _start(params, label):
            try:
                ...
                # Another session beat us to it (race against the should_run_audit
                # guard). Don't adopt their run — back out and let the next pass
                # render the "already running" notice.
                logger.info("%s blocked: an audit is already running", label)
                st.session_state["audit_run_requested"] = False
                st.rerun()
                return
            st.session_state["active_run_id"] = job.run_id
            # Owner token in the URL so a reconnect of THIS tab can re-attach.
            st.query_params["run"] = job.run_id
    ...
    if job is not None and not job.is_terminal():
        # This session owns the in-flight run — show the live progress panel (it
        # owns the Stop button + live checkpoint download). A reconnect of this
        # same tab re-attaches here via the URL owner token. (A run owned by a
        # DIFFERENT session is handled by the full-page gate near the top.)
        _audit_progress_panel()
    else:
        if job is not None:
            _finalize_job_into_session(job)
        # Idle/pre-run: poll so this session flips to the full-page busy gate
        # within ~2s if another session starts the one allowed audit.
        _collision_heartbeat()
